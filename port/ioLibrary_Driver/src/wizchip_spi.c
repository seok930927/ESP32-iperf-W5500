...
void spi_init_qspi(void)
{
    ...
    ret = spi_bus_initialize(SPI2_HOST, &buscfg, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_W5500_SPI, "SPI bus init failed: %s", esp_err_to_name(ret));
        return;
    }
    
    ESP_LOGI(TAG_W5500_SPI, "QSPI bus initialized successfully");
    
    // SPI 디바이스 추가
    ret = spi_bus_add_device(SPI2_HOST, &devcfg, &spi_dev);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_W5500_SPI, "SPI device add failed: %s", esp_err_to_name(ret));
        return;
    }

     ESP_LOGI(TAG_W5500_SPI, "QSPI device added successfully");
...
void spi_send_data(spi_device_handle_t spi, uint8_t *data, size_t len)
{
    ...
    ESP_LOGI(TAG_W5500_SPI, "SPI data sent successfully");
}
...
void spi_receive_data(spi_device_handle_t spi, uint16_t addr , uint8_t cmd ,  uint8_t *data, size_t len)
{
    ...
    if (total_len > sizeof(tx_rx_buffer)) {
        ESP_LOGE(TAG_W5500_SPI, "Data length too large. Max supported: %d bytes", 
    ...
    ESP_LOGI(TAG_W5500_SPI, "SPI command sent: 0x%02X, address: 0x%02X", 
             tx_rx_buffer[0], tx_rx_buffer[1]);
    ESP_LOGI(TAG_W5500_SPI, "SPI data received successfully");
}
...
{
    ESP_LOGI(TAG_W5500_SPI, "Starting W5500 hardware reset...");
    ...
    ESP_LOGI(TAG_W5500_SPI, "W5500 hardware reset completed");
}
